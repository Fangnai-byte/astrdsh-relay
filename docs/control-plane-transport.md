...
→ 迁移是**字段改名级别**（`events`→`records`，payload 包一层 `request` 并补 `address`/`throughSeq`），不是重写。`throughSeq` 的确切语义仍未实跑验证（§4.2）。
...
5. **`session.history` → `session/page` 的 `throughSeq` 语义**：结构对应已静态比对通过（§1.6.1 补充表），但**未实跑端到端回读**——`throughSeq` 是"翻页上界"还是"起始 seq"未验证；也未验证 `maxMessages` 在 `session/page` 与旧 `session.history` 中语义是否完全一致（connector 的整个回复等待循环 `_await_reply`（`dsh_client.py:441-495`）都压在这个调用上，实施时**必须**在 P1 阶段实测一次）。
